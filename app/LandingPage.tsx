...
export default function LandingPage({ html }: { html: string }) {
  ...
  useEffect(() => {
    ...
    if (header && !document.getElementById('rdp-theme-toggle')) {
      ...
      ;(targetRow as HTMLElement).appendChild(toggle)
    }
    ...
    const perfSection = Array.from(root.querySelectorAll('section')).find((section) =>
      section.textContent?.includes('Rock-solid performance, ready to deploy')
    ) as HTMLElement | undefined

    if (perfSection) {
      const filterButtons = Array.from(perfSection.querySelectorAll('button')) as HTMLButtonElement[]
      const searchInput = perfSection.querySelector('input[type="text"]') as HTMLInputElement | null
      const osHeading = Array.from(perfSection.querySelectorAll('h3')).find((el) => el.textContent?.trim() === 'Operating Systems') as HTMLElement | undefined
      const appsHeading = Array.from(perfSection.querySelectorAll('h3')).find((el) => el.textContent?.trim() === 'Applications') as HTMLElement | undefined
      const osCards = osHeading ? Array.from(osHeading.nextElementSibling?.querySelectorAll('a.group') || []) as HTMLElement[] : []
      const appsCards = appsHeading ? Array.from(appsHeading.nextElementSibling?.querySelectorAll('a.group') || []) as HTMLElement[] : []
      ...
      const applyFilter = () => {
        ...
        const updateButtonStyles = () => {
          filterButtons.forEach((btn) => {
            ...
            btn.className = isActive
              ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-200 scale-[1.02]'
              : 'bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/20 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-200'
          })
        }

        const updateCards = (cards: HTMLElement[], visible: boolean) => {
          cards.forEach((card) => {
        ...
        if (osHeading && osHeading.nextElementSibling) {
          ...
          ;(osHeading.nextElementSibling as HTMLElement).style.display = anyVisible ? '' : 'none'
        }
        if (appsHeading && appsHeading.nextElementSibling) {
          ...
          ;(appsHeading.nextElementSibling as HTMLElement).style.display = anyVisible ? '' : 'none'
        }
      }
    ...
    const faqSection = Array.from(root.querySelectorAll('section')).find((section) =>
    ...
    if (faqSection) {
      ...
      if (grid) {
        ...
        cards.forEach((item) => {
          ...
          const questionEl = trigger?.querySelector('span') as HTMLElement | null
          const question = questionEl?.textContent?.trim() || ''
          const answerText = faqAnswers[question]
          if (!trigger || !answerText) return

          trigger.classList.add('faq-trigger')
          const iconWrap = trigger.querySelector('span:last-child') as HTMLElement | null
          const iconSvg = trigger.querySelector('svg') as SVGElement | null
          if (iconWrap) iconWrap.classList.add('faq-toggle')
          if (iconSvg) iconSvg.classList.add('faq-toggle-icon')

          let answerWrap = item.querySelector('.faq-answer-wrap') as HTMLElement | null
          if (!answerWrap) {
          ...
          const answerContent = answerWrap.querySelector('.faq-answer-content') as HTMLElement | null
